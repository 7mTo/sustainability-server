...
test('ConnectionParameters construction', function() {
  ...
  assert.ok(new ConnectionParameters('postgres://10.254.144.101/postgres'), 'with connection string');
});
...
test('escape spaces if present', function() {
  subject = new ConnectionParameters('postgres://10.254.144.101/post gres');
  assert.equal(subject.database, 'post gres');
});

test('do not double escape spaces', function() {
  subject = new ConnectionParameters('postgres://10.254.144.101/post%20gres');
  assert.equal(subject.database, 'post gres');
});
...
test('libpq connection string building', function() {
  ...
  test('username or password contains weird characters', function() {
    ...
    defaults.ssl = true;
    var strang = 'pg://my f%irst name:is&%awesome!@10.254.144.101:9000';
    ...
    assert.equal(subject.host, '10.254.144.101');
    assert.equal(subject.ssl, true);
  });

  test("url is properly encoded", function() {
    var encoded = "pg://bi%25na%25%25ry%20:s%40f%23@10.254.144.101/%20u%2520rl";
